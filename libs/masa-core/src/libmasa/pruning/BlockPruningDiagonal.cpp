...
void BlockPruningDiagonal::updatePruningWindowMulti(int diagonal, const score_t* block_scores) {
        ...
        windowStart = 0;
        // windowEnd = getGrid()->getGridWidth();
        ...
        if  (diagonal >  ((getGrid()->getGridHeight()/2) - (splitstep-1)*getGrid()->getGridWidth())) {
                ...
                //printf ("*** \n");
                bool isLastBlockPrunable;
                //if (block_scores[windowStart].score > 40050427)
                //   printf ("\n\n\n 1: ### bx: %d , by: %d , score: %d , diag: %d ### \n\n", windowStart, diagonal-windowStart, block_scores[windowStart].score,diagonal);
                ...
                //} else {
                   ...
                   while (windowStart < windowEnd) {
                        ...
                        //if (block_scores[bx].score > 40050427)
                        //      printf ("\n\n\n 2: ### bx: %d , by: %d , score: %d , diag: %d ### \n\n", bx, by, block_scores[bx].score,diagonal);
...
//if (DEBUGM) fprintf(dbbpd, "111 ** Diag: %d - start: %d - end: %d - width: %d - heigth: %d ** bx: %d - by: %d - score: %d \n", diagonal, windowStart, windowEnd,getGrid()->getGridWidth(), getGrid()->getGridHeight(), bx, by, block_scores[bx].score);
       ...
       // The right-pruning for rectangular blocks must be redesigned, since we
        ...
        if (windowEnd < windowStart) {
                ...
                //if (block_scores[windowEnd].score > 40050427)
                //    printf ("\n\n\n 3: ### bx: %d , by: %d , score: %d , diag: %d ### \n\n", windowEnd, diagonal-windowEnd, block_scores[windowEnd].score,diagonal);
