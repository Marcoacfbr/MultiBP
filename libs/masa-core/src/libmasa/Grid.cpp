...
Grid::Grid(Partition partition) {
	...
	this->seq1_size = partition.seq1_size;
...
void Grid::splitGridHorizontally(int count) {
	...
	//printf("\n\n Count: %d, Width: %d, minBlockWidth: %d", count, width, minBlockWidth);
	...
	if (this->split) {
		...
		this->blockSplitHorizontalMulti = new int[513];
		//AlignerUtils::splitBlocksEvenly(this->blockSplitHorizontalMulti, 0, this->seq1_size, 512);
                AlignerUtils::splitBlocksEvenly(this->blockSplitHorizontalMulti, jstart, jend, 512);
                //printf ("\n ***  %d - %d ***",  jstart, jend);
	}
	...
	for (int i=0; i<count+1;i++)
		printf ("%d - ", blockSplitHorizontal[i]);
	printf ("*** \n"); */
}
...
void Grid::getBlockPositionHMulti(int bx, int by, int* j0, int* j1) const {
	if (bx < 0 || by < 0 || bx >= blockCountHorizontal || by >= blockCountVertical) {
		if (j0 != NULL) *j0 = -1;
		if (j1 != NULL) *j1 = -1;
	} else {
		if (blockSplitHorizontalMulti != NULL) {
			if (j0 != NULL) *j0 = blockSplitHorizontalMulti[bx];
			if (j1 != NULL) *j1 = blockSplitHorizontalMulti[bx+1];
			//printf("Estou no IF\n");
		} else {
			printf("Estou no ELSE\n");
			if (j0 != NULL) *j0 = partition.getJ0() + bx*blockWidth;
...
void Grid::getBlockPositionMulti(int bx, int by, int* i0, int* j0, int* i1, int* j1) const {
	getBlockPositionHMulti(bx, by, j0, j1);
	getBlockPositionV(bx, by, i0, i1);
}
