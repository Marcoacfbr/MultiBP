...
-t, --trim=I0,I1,J0,J1  Trims sequence #1 from position I0 to I1 (inclusive).  \n\
                           ...
                           --split parameter. \n\
--dynamic=COUNT         Performs dynamic load balance                          \n\
...
static void split_sequences ( Job* _job, int split_step, int split_count, int* weights, int wait_step ) {
    ...
    if (split_step % dynamic == 1) {
      	splitdyn = split_step + dynamic - 1;
        if (splitdyn == split_count)
            leftdyn = -1;
        else
      	    leftdyn =  ( int ) ( ( (( long long int ) seq1_len)*proportions[splitdyn] ) /sum );
     }
     else if (split_step % dynamic == 0) {
       splitdyn = split_step - dynamic + 1;
       rightdyn =  ( int ) ( ( (( long long int ) seq1_len)*proportions[splitdyn] ) /sum ); 
     }
...
int libmasa_entry_point(int argc, char** argv, IAligner* aligner, char* aligner_header) {
    ...
    static struct option long_options[] = {
        ...
        {"dynamic",     required_argument,      0, ARG_DYNAMIC},
		...
		while ( 1 ) {
			...
			case ARG_PART:
				...
				break;
                        case ARG_DYNAMIC:
				if ( optarg != NULL )  {
					sscanf ( optarg, "%d", &dynamic);
                                } else {
					throw IllegalArgumentException("Inform --dynamic parameter.", current_arg);
				}
				break;
			case ARG_CLEAR_N:
				clear_n = true;
