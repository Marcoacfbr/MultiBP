...
void AlignerManager::alignPartition(Partition partition, int start_type) {
	int ps = partition.split;
	int s1size = partition.seq1_size;
...
//	}
	...
	if (this->active) {
		//printf ("\n\n\ #### Entrei no this->active ### \n\n");
		Partition partition_adj = Partition(partition, -seq0_offset, -seq1_offset);
		partition_adj.split = ps;
		partition_adj.seq1_size = s1size;
...
void AlignerManager::setmustSplit(bool split) {
	this->mustsplit = split;
}
